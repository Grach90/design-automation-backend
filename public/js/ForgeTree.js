...
function initFile() {
  getForgeToken(async function (access_token) {
    const response = await fetch(
      `http://localhost:4000/api/forge/fileinitialize/revit_sample_file.rvt`,
      {
        method: "GET",
        headers: {
...
function deletBucket() {
  getForgeToken(function (access_token) {
    const test = jQuery.ajax({
      url: `http://localhost:4000/api/forge/deletebucket`,
      data: access_token,
      dataType: "GET",
...
function translateObject(node) {
  ...
  const file_format = node.text.split(".")[1];
  if (file_format == "zip") {
    const rootFile = prompt("Enter the Root file name (example.iam)", "");
    jQuery.post({
